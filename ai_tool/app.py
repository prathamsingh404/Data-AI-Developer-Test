...
# Database path
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "growify.db")
...
def get_llm_response(prompt):
    ...
    if not api_key:
        return "⚠️ Gemini API key is missing. Please set the GEMINI_API_KEY environment variable.", None
...
1. campaign_performance:
   ...
   - "Data Source name" (TEXT, values: "Brand A", "Brand B")
   - "Campaign Name" (TEXT)
   - "Campaign Effective Status" (TEXT, values: "ACTIVE", "PAUSED")
   - "Ad Set Name" (TEXT)
   - "Ad Name" (TEXT)
   - "Country Funnel" (TEXT, target country)
   - "Geo Location Segment" (TEXT)
   - "FB Spent Funnel (INR)" (REAL)
   - "Amount Spent (INR)" (REAL, primary ad spend)
   - "Clicks (all)" (REAL)
   - Impressions (REAL)
   - "Page Likes" (REAL)
   ...
   - "Purchases Conversion Value (INR)" (REAL, ad conversion revenue value)
   ...
   - ROI (REAL, ROI = Purchases Conversion Value / Spend)
   - Brand (TEXT, e.g. "Growify")
   - "Funnel Stage" (TEXT, values: "TOF", "MOF", "BOF", "Retarget", "MOF+BOF", "Unknown")
   - Region (TEXT, e.g. "India", "United States", "United Kingdom", "United Arab Emirates", "Canada", "Australia")
   - "Adset Channel" (TEXT, values: "Walkins", "Whatsapp", "Ecomm", "Brand Visibility", "Market Places", "Unknown")
   - "Adset Target" (TEXT, values: "LAL", "Engaged Shoppers", "Open", "Retargeting", "Unknown")
   - "Ad Source Type" (TEXT, values: "CA", "EP", "Catalogue")
   - "Ad Format" (TEXT, values: "Video", "SI", "Carousel", "Flexible", "Collection", "Unknown")
   - "Ad Concept" (TEXT, e.g. "Influencer", "UGC", "StoreVideo", "CampaignShoot", "Brand Creative")
   - "Ad Category" (TEXT, e.g. "Kurta", "Festive", "Saree", "Dress", "EOSS", "Unknown")
   - "Ad Collection" (TEXT, e.g. "Womens", "Mens", "Apparel", "Stoles", "Best Seller", "General")
...
2. shopify_sales:
   ...
   - "Net Sales (INR)" (REAL)
   - "Total Sales (INR)" (REAL, actual sales revenue)
   - Orders (REAL, 1.0 for valid order, 0.0 for return/refund)
   - "Returns (INR)" (REAL)
   - "Return Rate" (REAL, Returns / Gross Sales)
   - "Items Sold" (REAL)
   - "Items Returned" (REAL)
   - "Average Order Value (INR)" (REAL, Total Sales / Orders)
   - "Discounts (INR)" (REAL)
   - SKU (TEXT)
   - "Customer Sale Type" (TEXT, values: "First-Time", "Returning")
   - "Customer ID" (INTEGER)
   - "Billing Country" (TEXT)
   - "Shipping Country" (TEXT, primary market locator)
...
2. Ensure you query the correct columns. Note that table column names containing spaces or special characters must be enclosed in double quotes (e.g. "Amount Spent (INR)" or "Funnel Stage").
...
if db_exists:
    ...
    st.warning("⚠️ Cleaned database file 'growify.db' was not found in 'data/' directory. Please run the clean_data.py pipeline first.")
...
"""
                ...
                if corrected_sql:
                    ...
                    with st.expander("🛠️ Corrected SQL Query", expanded=True):
                        st.code(sql_query, language="sql")
                    query_results, query_error = run_query(sql_query)

...
- Corrected 2705 negative value signs across numeric fields (assuming sign inversion).
...
4. **Sign Corrections**: Negative metrics corrected using absolute values ONLY where business logic indicated a platform sign inversion. Returns/refunds were preserved as logical deductions.
5. **Star Schema Parse**: Expanded Campaign Name, Ad Set Name, and Ad Name fields using regex-based keyword matching.
6. **Metric Recalculations**: Recomputed CTR (as %), CPC, CPM, and ROAS in campaigns, and Net Sales, Total Sales, Return Rate, and AOV in Shopify sales. Validated metrics with assertions.
7. **Schema Enforcement**: Explicitly created tables using DDL to enforce constraints before appending data.
